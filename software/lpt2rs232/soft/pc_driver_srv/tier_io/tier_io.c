...
//
// normalizacja poziomow predkosci
//
static int spd2pwm(int v)
{
  const double A=255;
  const double B=255;
  return (int)( A*sqrt(v/B) );
}; // spd2pwm()

// wyrownanie rzeczywistej predkosci gasiennic:
#define MIN(a,b)    ( ((a)<(b))?(a):(b) )
#define MAX(a,b)    ( ((a)>(b))?(a):(b) )
static void spd_eq(int *left, int *right)
{
  const int d=120;
...
int tier_io_spdst(struct tier_io *tio, int left, int right)
{
 ...
 // transform to final value
 t1=spd2pwm(t1);
 t2=spd2pwm(t2);

 // there is a forbiden range...
 const int min=180;
 if(0<t1 && t1<min)
   t1=min;
 if(0<t2 && t2<min)
