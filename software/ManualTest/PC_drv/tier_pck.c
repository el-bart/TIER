...
struct tier_pck_all tier_pck_mk_init(void)
{
 ...
 pck.chk_sum=cnt_chk_sum(1+0, pck_char);
...
struct tier_pck_all tier_pck_mk_pp(void)
{
 ...
 pck.pck.pp.seq_no=(unsigned char)time(NULL);	// jakas przypadkowa liczba
 pck_char[1]      =pck.pck.pp.seq_no;
...
struct tier_pck_all tier_pck_mk_stop(void)
{
 ...
 pck.pck.stop.byte1=pck.type+0x25;		// 0x15
 pck_char[1]       =pck.pck.stop.byte1;
 pck.pck.stop.byte2=pck.pck.stop.byte1+0x59;	// 0x6e
 pck_char[2]       =pck.pck.stop.byte2;

 pck.chk_sum=cnt_chk_sum(1+2, pck_char);	// 0x76
                       ...
                       const struct tier_pck_all *pck)
{
 ...
 switch(pck->type)
 {
   ...
   case TIER_PCK_TYPE_PP:
        serial->byte[ serial->size++ ]=pck->pck.pp.seq_no;
        break;
                         ...
                         const struct tier_pck_serial *serial)
{
 ...
 switch(pck->type)
 {
   ...
   case TIER_PCK_TYPE_PP:
        pck->pck.pp.seq_no=serial->byte[ cnt++ ];
        break;
 ...
 switch(cmd->type)
 {
   ...
   case TIER_PCK_TYPE_PP:
        return rep->type         ==TIER_PCK_TYPE_PP &&
               cmd->pck.pp.seq_no==(unsigned char)(rep->pck.pp.seq_no-1);
