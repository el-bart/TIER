...
//
// transformacja skalujaca wartosci 0-255 na
// moc (w sensie wypelnienia PWM) oraz transformacja
// do niej odwrotna (dla odczytu).
//
// uwaga: ze wzgledu na skonczona dokladnosc w ogolnosci
//        pwm2speed( speed2pwm(v) ) != v
//

#define AMP   241     // amplituda
#define B     AMP     // wspolczynnik proporcjonalnosci - tu rowny amplitudzie
#define K     2.5     // wspolczynnik stromosci transformacji

static unsigned char speed2pwm(unsigned char v)
{
  double vn=(AMP<v)?AMP:v;   // v znormalizowane
  //return (unsigned char)( AMP*(1-1.0/(K*vn)) );
  return (unsigned char)( AMP*sqrt( vn/B ) );
}; // speed2pwm()

static unsigned char pwm2speed(unsigned char v)
{
  //return (unsigned char)( v/(K*AMP-K*v*1.0) );
  double w=(1.0*v)/AMP;
  return (unsigned char)( B*w*w );
}; // pwm2speed()
...
int tier_io_spdst(struct tier_io *tio, int left, int right)
{
 ...
 if( 256<=abs(left) || 256<=abs(right) )
   return -1;

 t2 =speed2pwm( abs(left ) );
 t1 =speed2pwm( abs(right) );
...
{
 assert(!"tier_io_spdrd(): not implemented");
 //unsigned char t1,t2;
 //unsigned char dir;
